...
public class ScreenshotManager {
    ...
    private static void registerAndAdd(NativeImage image, String filename, Minecraft client) {
        int thumbHeight = (int) ((float) image.getHeight() / image.getWidth() * THUMBNAIL_WIDTH);
        Identifier id = Identifier.fromNamespaceAndPath("strayers-skyblock-utils", "screenshot_" + counter++);
    ...
    public static void buildScreenshotHud(GuiGraphicsExtractor context) {
        if (screenshots.isEmpty()) return;

        Minecraft client = Minecraft.getInstance();
        int screenWidth = client.getWindow().getGuiScaledWidth();
        int mouseX = (int) (client.mouseHandler.xpos() * client.getWindow().getGuiScaledWidth() / client.getWindow().getWidth());
        ...
        for (int i = 0; i < screenshots.size(); i++) {
            ...
            context.enableScissor(screenWidth - THUMBNAIL_WIDTH - GAP - 2, 0, screenWidth, client.getWindow().getGuiScaledHeight());
            ...
            context.blit(entry.textureId(), x, y, x + THUMBNAIL_WIDTH, y + entry.thumbHeight(), 0f, 1f, 0f, 1f);
    ...
    public static void tick() {
        ...
        for (int i = screenshots.size() - 1; i >= 0; i--) {
            if (screenshots.get(i).expired()) {
                client.getTextureManager().release(screenshots.get(i).textureId());
