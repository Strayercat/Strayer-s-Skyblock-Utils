...
public class WaypointRenderer {
    ...
    static final RenderPipeline BADGE_PIPELINE = RenderPipelines.register(
            RenderPipeline.builder(RenderPipelines.POSITION_COLOR_SNIPPET)
                    .withLocation(Identifier.of("skyblockutils", "pipeline/waypoint_badge"))
    ...
    private static final RenderLayer BADGE_BACKGROUND = RenderLayer.of(
            ...
            RenderSetup.builder(BADGE_PIPELINE).build()
    );
    ...
    private static void renderBadge(Waypoint waypoint, double distance, Vec3d cameraPos, Camera camera, TextRenderer font, VertexConsumerProvider.Immediate immediate) {
        ...
        IrisCompat.beginCustomDraw();

        VertexConsumer outline = immediate.getBuffer(BADGE_BACKGROUND);
        outline.vertex(pose, -badgeW - o, -badgeH - o, 0).color(outlineColor);
        outline.vertex(pose, badgeW + o, -badgeH - o, 0).color(outlineColor);
        outline.vertex(pose, badgeW + o, o, 0).color(outlineColor);
        outline.vertex(pose, -badgeW - o, o, 0).color(outlineColor);
        outline.vertex(pose, -badgeW - o, o, 0).color(outlineColor);
        outline.vertex(pose, badgeW + o, o, 0).color(outlineColor);
        outline.vertex(pose, 0, tipH + o, 0).color(outlineColor);
        outline.vertex(pose, 0, tipH + o, 0).color(outlineColor);
        immediate.draw(BADGE_BACKGROUND);

        VertexConsumer fill = immediate.getBuffer(BADGE_BACKGROUND);
        fill.vertex(pose, -badgeW, -badgeH, 0).color(bgColor);
        fill.vertex(pose, badgeW, -badgeH, 0).color(bgColor);
        fill.vertex(pose, badgeW, 0, 0).color(bgColor);
        fill.vertex(pose, -badgeW, 0, 0).color(bgColor);
        fill.vertex(pose, -badgeW, 0, 0).color(bgColor);
        fill.vertex(pose, badgeW, 0, 0).color(bgColor);
        fill.vertex(pose, 0, tipH, 0).color(bgColor);
        fill.vertex(pose, 0, tipH, 0).color(bgColor);
        immediate.draw(BADGE_BACKGROUND);

        IrisCompat.endCustomDraw();
